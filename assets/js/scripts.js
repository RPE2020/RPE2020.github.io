...
function identification(from)
{
	...
	$('#load').show();
	var d = new Date();
	if($('#stay').is(":checked"))var n = Math.trunc((d.getTime()+100*24*3600*1000)/1000); /* Connecté pour 100 jours */
...
function reset_pass()
{
	...
	$.getJSON("./api/ajax/change_passwd?mode=genere-token&mail="+mail).done(function(data){
		if(data.error==0)$('#form_pass').html("Mail de récupération envoyé ! Pensez à vérifier les spams");
		else
		{
...
function contact()
{
	...
	$.post('./assets/php/contact.php', {nom:nom,mail:mail,msg:msg},function(data){
